...
Rotina que irá buscar no Web Service do SIENGE os pedidos de compras a serem processados na empresa logada
@type user function
@version 12.1.27
@author elton.alves@totvs.com.br
@since 17/07/2020
/*/
user function SiengeV3()

    //Parâmetros utilizados por todos os End Points dos Web Service do SIENGE
    Private cUrl       := AllTrim( GetMv( 'SIE_URL' ) )
    Private cPrefix    := AllTrim( GetMv( 'SIE_SUBDOM' ) )
    Private aHeader    := { 'Authorization: Basic ' + Encode64( AllTrim( GetMv( 'SIE_USER' ) ) + ':' + AllTrim( GetMv( 'SIE_PASWOR' ) ) ) }
    Private cCodSienge := GetMv( 'SIE_CODEMP' )
