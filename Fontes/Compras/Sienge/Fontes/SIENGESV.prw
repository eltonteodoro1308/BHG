...
que tem como objetivo buscar no Web Service do SIENGE os Serviços no end point de Títulos a Pagar 
nele cadastrados e importá-los para base do Protheus como um Pedido de Compras
@type user function
@version 12.1.27
...
User Function SiengeSv()
    ...
    @ 002, 002 GROUP oGrpData TO 072, 096 PROMPT "Req. P/ Data do Serviço e Docto:" OF oDlgMain COLOR 0, 16777215 PIXEL
    ...
    @ 002, 099 GROUP oGrpId TO 073, 202 PROMPT "Requisitar por ID do Serviço" OF oDlgMain COLOR 0, 16777215 PIXEL
...
Requista Serviços em uma faixa de datas e docuemnto
@type static function
@version 12.1.27
@author elton.alves@totvs.com.br
@since 02/09/2020
...
@param cDocto, character, Número do Documento a ser rquisirado
/*/
static function ProcData( dDe, dAte, cDocto )
    ...
    elseif Empty( dDe ) .Or. Empty( dAte )
        ...
        MsgRun ( 'Buscando Serviços no Web Service do SIENGE...', 'Aguarde alguns instantes...', {|| aCabPedCmp := RunData( dDe, dAte, cDocto ) } )

    end if

    if Empty( aCabPedCmp )

        ApMsgAlert( 'Não houve retorno de Serviços na busca solicitada.', 'SIENGE' )
...
static function ProcId( cID )
    ...
    if Empty( cId )
        ...
        ApMsgAlert( 'Informe um Id de Serviço Válido.', 'SIENGE' )

    else

        MsgRun ( 'Buscando Serviços no Web Service do SIENGE...', 'Aguarde alguns instantes...', {|| aCabPedCmp := RunId( cID ) } )
...
static function RunId( cID )
    ...
    if lOk
        ...
        if oJson['documentIdentificationId'] != 'PCT'
            ...
            ApMsgAlert(  'Serviço "' + AllTrim( cID ) + '" não é do tipo "PCT"', 'SIENGE' )

        elseif cValtoChar( oJson["debtorId"] ) != cSIEEMPRES

            ApMsgAlert(  'Serviço "' + AllTrim( cID ) + '" não pertence a esta Empresa/Filial', 'SIENGE' )
    ...
    elseif "Resource not found" $ oJson['developerMessage']
        ...
        ApMsgAlert( 'Serviço "' + AllTrim( cID ) + '" não localizado', 'SIENGE' )
...
Busca no SIENGE o CNPJ do fornecedor do Serviço
@type static function
@version 12.1.27
@author elton.alves@totvs.com.br
@since 14/09/2020
...
static function ItensOk( oJsonCabec )
    ...
    if Empty( oJsonCabec['itens'][1]['B1_COD'] )
        ...
        oJsonCabec['itens'][1]['situacao'] := 'Serviço não informado no cadastro do Fornecedor'
        ...
        if SB1->( DbSeek(  xFilial() + oJsonCabec['itens'][1]['B1_COD']  ) )
            ...
            if SB1->B1_MSBLQL == '1'
                ...
                if Empty( SB1->B1_CONTA )
                    ...
                    oJsonCabec['itens'][1]['situacao'] := 'Produto sem Conta Contábil'
                    ...
                    if CT1->( DbSeek( SB1->( xFilial('CT1') + B1_CONTA ) ) )
                        ...
                        if CT1->CT1_MSBLQL == '1'
                            ...
                            oJsonCabec['itens'][1]['situacao'] := 'Conta Contábil Bloqueada.'
                        ...
                        oJsonCabec['itens'][1]['situacao'] := 'Conta Contábil não Localizada.'
            ...
            oJsonCabec['itens'][1]['situacao'] := 'Não Localizado no Cadastro'
            oJsonCabec['itens'][1]['B1_DESC']  := ''
...
static Function ShowPedCmp( aCabPedCmp )
    ...
    DEFINE MSDIALOG oDlg TITLE "Serviços - SIENGE" FROM 000, 000  TO 500, 750 COLORS 0, 16777215 PIXEL
    ...
    @ 002, 082 BUTTON oBtnLegenda PROMPT "Legenda" SIZE 037, 012 OF oDlg ACTION BrwLegenda( 'Situação dos Serviços', '', aLegenda) PIXEL
...
static function PrcPedCmp( oBrwLstPed, aCabPedCmp )
    ...
    elseif aScan( oBrwLstPed:aArray, { |item| item[1] } ) == 0
        ...
        ApMsgAlert( 'Nenhum Serviço foi selecionado para ser importado', 'SIENGE' )
