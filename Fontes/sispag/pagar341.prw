...
Do Case
    ...
    Case _cTipo == "PP001"	//  Agencia e Conta Corrente Favorecido
       ...
       If sa2->a2_banco == "341"
          ...
          _cRetorno += strzero(val(substr(_cConta,6,5)),5)+" "+substr(sa2->A2_DVAGE,1,1)                               
       Else
          _cRetorno := strzero(val(substr(sa2->a2_agencia,1,5)),5)+" "
          _cRetorno += strzero(val(substr(_cConta,1,12)),12)
          If len(alltrim(sa2->A2_DVAGE)) > 1
            _cRetorno += strzero(val(substr(sa2->A2_DVAGE,1,2)),2)
          Else
             _cRetorno += " "+substr(sa2->A2_DVAGE,1,1)    	
          EndIf                                     
       EndIf
    ...
    Case _cTipo == "PT001"	                           
         ...
         If SEA->EA_MODELO == "16"    
           ...
           If !Empty(SE2->E2_CNPJRET)
               _cRetorno += Iif (len(alltrim(SE2->E2_CNPJRET))>11,"2","1")
           Else               
              _cRetorno += "2"       
           EndIf
         ...
         ElseIf (SEA->EA_MODELO == "17")   
            ...
            _cRetorno += STRZERO(MONTH(SE2->E2_DTAPUR),2)+STRZERO(YEAR(SE2->E2_DTAPUR),4)
          //  _cRetorno += STRZERO(Val(SE2->E2_XCPGPS),6)
         ...
         ElseIf SEA->EA_MODELO == "35" 
            ...
            // Posicao 024 a 024: Tp Inscricao  1-CNPJ /  2-CEI   (CORRIGIDO)            
            If !Empty(SE2->E2_CNPJRET)
                _cRetorno += Iif (len(alltrim(SE2->E2_CNPJRET))>11,"1","2")
            Else               
              _cRetorno += "2"       
            EndIf
